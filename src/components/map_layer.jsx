...
const MAPBOX_ACCESS_TOKEN =
  "pk.eyJ1IjoieWF3b2t5ZXJlIiwiYSI6ImNrb2FpMzVocjBkYjUyb3FtdmdrbXEwYW4ifQ.qNlA0ASU5idyi6cr5Twg_w";
...
const Map = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await fetch(
        "https://tiger.ridsi-dash.com:8000/jeffcity.geojson",
    ...
    fetchData().catch((err) => console.error("Could not load data", err));
  }, []);
  ...
  const onClick = (event) => {
    ...
    const filt_data = data.features.filter((element) => {
      if (
        element["properties"]["street_name"] === street_name &&
        element["properties"]["pci"] <= pci_thresh
      ) {
        return true;
      }
      return false;
    ...
    filt_data.map((road_feat) => {
      ...
      for (var i = 0; i < dist_types.length; i++) {
        if (distresses.indexOf(dist_types[i]) === -1) {
          distresses.push(dist_types[i]);
        }
      }
    });
    ...
    const filt_reg = data_ori.features.filter((element) => {
      ...
      if (
        geolib.isPointInPolygon(
          [clon, clat],
          plg["features"][0].geometry.coordinates[0]
        )
      ) {
        return true;
      }
  ...
  const drillStreetName = (streetName) => {
    console.log(streetName);
    console.log(data_ori);
    const firstTwoWords = streetName
      .toLowerCase()
      .split(" ")
      .slice(0, 2)
      .join(" ");
    console.log(firstTwoWords);
    const filteredData = data_ori.features.filter((element) => {
      const streetNameFromData = element["properties"]["street_name"]
        .toLowerCase()
        .split(" ")
        .slice(0, 2)
    ...
    if (filteredData.length === 0) {
      ...
      alert(`No features found for street name '${streetName}'`);
      return;
    } else {
  ...
  return (
    <div className="Main">
      <div className="map-header">EXPLORER</div>
      {show ? (
      ...
      <div className="Map">
        <SideDiv
          ...
          onStlChange={(value) => setSatellite(value)}
          onStreetChange={(value) => setSearchValue(value)}
          alchecked={alchecked}
          onAlchange={(value) => {
            setAlchecked(value);
            value
              ? filter_crack_type("Alligator Cracking")
              : unfilter_crack_type(pci_thresh);
          }}
          blchecked={blchecked}
          onBlchange={(value) => {
            setBlchecked(value);
            value
              ? filter_crack_type("Block Cracking")
              : unfilter_crack_type(pci_thresh);
          }}
          tlchecked={tlchecked}
          onTlchange={(value) => {
            setTlchecked(value);
            value
              ? filter_crack_type("Transverse Cracking")
              : unfilter_crack_type(pci_thresh);
          }}
          llchecked={llchecked}
          onLlchange={(value) => {
            setLlchecked(value);
            value
              ? filter_crack_type("Longitudinal Cracking")
              : unfilter_crack_type(pci_thresh);
          }}
          plchecked={plchecked}
          onPlchange={(value) => {
            setPlchecked(value);
            value
              ? filter_crack_type("Pothole")
              : unfilter_crack_type(pci_thresh);
          }}
          pccchecked={pccchecked}
          onPcchange={(value) => {
            setPcchecked(value);
            value
              ? filter_crack_type("PCC_joint")
              : unfilter_crack_type(pci_thresh);
          }}
          spalchecked={spalchecked}
          onSpchange={(value) => {
            setSpchecked(value);
            value
              ? filter_crack_type("Spalling")
              : unfilter_crack_type(pci_thresh);
          }}
          ...
          onRfchange={(value) => {
            setRfchecked(value);
            value
              ? filter_crack_type("Reflective Cracking")
              : unfilter_crack_type(pci_thresh);
          }}
          ravchecked={ravchecked}
          onRachange={(value) => {
            setRachecked(value);
            value
              ? filter_crack_type("Raveling")
              : unfilter_crack_type(pci_thresh);
          }}
          ...
          onPopchange={(value) => {
            setPopchecked(value);
            value
              ? filter_crack_type("Pop_out")
              : unfilter_crack_type(pci_thresh);
          }}
          patchchecked={patchchecked}
          onPatchchange={(value) => {
            setPatchchecked(value);
            value
              ? filter_crack_type("Patch")
              : unfilter_crack_type(pci_thresh);
          }}
          manchchecked={manchchecked}
          onManchchange={(value) => {
            setManchchecked(value);
            value
              ? filter_crack_type("Manhole")
              : unfilter_crack_type(pci_thresh);
          }}
          pci_thresh={pci_thresh}
          onPctchange={(value) => {
            setPcithresh(value);
            filter_data(value);
          }}
          onFilterCheck={() => clearFilters()}
          filterByStreetName={() => drillStreetName(searchValue)}
        />
          ...
          >
            <div style={{ width: "200px" }}>
              ...
              <div style={{ flex: 1, alignItems: "left" }}>
                Section PCI: {Math.round(urlpci)}
              </div>
              <div style={{ flex: 1, alignItems: "left" }}>
                Image PCI: {Math.round(cpci)}
              </div>
              🚧
            </div>
          </Popup>
      ...
      <LoadMore
        show={show}
        update={update}
        onUpdate={(value) => setUpdate(value)}
      />
    </div>
  );
};
